...
% n_d; number of dimension 决策变量
% n_c: number of clusters 5
% rang_l; left boundary of the dynamic range
% rang_r; right boundary of the dynamic range
prob_one_cluster = 0.8; % 选择聚类概率 probability for select one cluster to form new individual; 
stepSize = ones(1,n_d); % 步长 effecting the step size of generating new individuals by adding random values
popu = rang_l + (rang_r - rang_l) * rand(n_p,n_d); % 种群 initialize the population of individuals
popu_sorted  = rang_l + (rang_r - rang_l) * rand(n_p,n_d); %  initialize the  population of individuals sorted according to clusters
n_iteration = 0; % 当前迭代次数 current iteration number
% initialize cluster probability to be zeros
prob = zeros(n_c,1);
best = zeros(n_c,1);  % 聚类中最好个体 index of best individual in each cluster
centers = rang_l + (rang_r - rang_l) * rand(n_c,n_d);  % 聚类中最好个体 initialize best individual in each cluster
...
best_fitness = 1000000*ones(max_iteration,1);  %最好适应度
fitness_popu = 1000000*ones(n_p,1);  % 适应度值排序 store fitness value for each individual
fitness_popu_sorted = 1000000*ones(n_p,1);  % store  fitness value for each sorted individual
indi_temp = zeros(1,n_d);  %子代 store temperary individual
%**************************************************************************
%**************************************************************************
...
while n_iteration < max_iteration
    ...
    % clustering    
    fit_values = 100000000000000000000000000.0*ones(n_c,1);  % 聚类最小值assign a initial big fitness value  as best fitness for each cluster in minimization problems
    number_in_cluster = zeros(n_c,1);  %聚类中个体数目 initialize 0 individual in each cluster      
    for idx = 1:n_p
